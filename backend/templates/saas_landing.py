...
def render(spec: dict) -> dict[str, str]:
    ...
    icon_svg = {
        ...
        "users": '<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.856-1.487M15 10a3 3 0 11-6 0 3 3 0 016 0zM12.93 12a7 7 0 00-6.86 0"/></svg>',
        ...
        "shield": '<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>',
    ...
    for feat in features[:6]:
        ...
        features_html += f"""
    <div class="rounded-2xl border border-neutral-800 p-8 hover:border-neutral-700 transition-colors">
      <div class="mb-4 text-{accent_light}">{icon}</div>
      <h3 class="text-lg font-semibold text-neutral-50 mb-2">{feat.get('title', 'Feature')}</h3>
    ...
    <div class="text-center">
      <div class="text-4xl font-bold text-{accent_light} mb-2">{metric.get('value', '0')}</div>
      <p class="text-neutral-400">{metric.get('label', '')}</p>
    </div>"""
    ...
    for plan in pricing_plans[:3]:
        ...
        pricing_html += f"""
    <div class="{'ring-2 ring-' + accent_light + ' scale-105' if is_highlighted else 'border border-neutral-800'} rounded-2xl p-8 {'bg-neutral-800 bg-opacity-40' if is_highlighted else ''}">
      <h3 class="text-2xl font-bold text-neutral-50 mb-2">{plan.get('name', 'Plan')}</h3>
      <p class="text-neutral-400 mb-4">{plan.get('description', '')}</p>
      <div class="mb-6">
        <span class="text-4xl font-bold text-neutral-50">{plan.get('price', 'Custom')}</span>
        <span class="text-neutral-400 ml-2">{plan.get('period', '')}</span>
      </div>
      <ul class="space-y-3 mb-8">"""
        for feature in plan.get("features", [])[:3]:
            pricing_html += f'<li class="text-neutral-300 flex items-center"><span class="mr-3 text-{accent_light}">✓</span>{feature}</li>'
        pricing_html += f"""
      </ul>
      <button class="w-full bg-{accent_light} hover:bg-{accent_dark} text-neutral-950 font-semibold py-3 rounded-lg transition-colors">{plan.get('cta', 'Get started')}</button>
    </div>"""
    ...
    <section class="min-h-screen bg-neutral-950 flex items-center justify-center px-6 py-32">
      <div class="max-w-6xl mx-auto">
        <div class="text-center mb-12">
          <p class="text-{accent_light} text-sm font-semibold uppercase tracking-widest mb-4">{product_name}</p>
        ...
        <div class="rounded-2xl overflow-hidden border border-neutral-800">
          <img src="https://images.unsplash.com/photo-1552664730-d307ca884978?w=1600&q=80&auto=format&fit=crop" alt="Product demo" class="w-full h-auto" />
        </div>
      </div>
    </section>
  );
    ...
    <section class="py-24 bg-neutral-950 px-6">
      <div class="max-w-6xl mx-auto">
        <div class="text-center mb-16">
          <h2 class="text-4xl md:text-5xl font-bold tracking-tight text-neutral-50 mb-4">Powerful features</h2>
          <p class="text-lg text-neutral-400">Everything you need to build faster and smarter</p>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">{features_html}
        </div>
      </div>
    </section>
  );
...
function Metrics() {{
  return (
    <section class="py-24 bg-neutral-900 px-6">
      <div class="max-w-6xl mx-auto">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-12">{metrics_html}
        </div>
      </div>
    </section>
  );
  ...
  return (
    <section class="py-24 bg-neutral-950 px-6">
      <div class="max-w-4xl mx-auto text-center">
        <p class="text-2xl md:text-3xl font-light text-neutral-100 mb-8 leading-relaxed">"{testimonial['quote']}"</p>
        <div class="flex items-center justify-center gap-4">
          <div class="w-12 h-12 rounded-full bg-{accent_light} opacity-20"></div>
          <div>
            <p class="font-semibold text-neutral-50">{testimonial['author']}</p>
            <p class="text-sm text-neutral-400">{testimonial['role']} at {testimonial['company']}</p>
          </div>
        </div>
      </div>
    </section>
...
function Pricing() {{
  return (
    <section class="py-24 bg-neutral-950 px-6">
      <div class="max-w-6xl mx-auto">
        <div class="text-center mb-16">
          <h2 class="text-4xl md:text-5xl font-bold tracking-tight text-neutral-50 mb-4">Simple, transparent pricing</h2>
          <p class="text-lg text-neutral-400">Choose the plan that fits your needs</p>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">{pricing_html}
        </div>
      </div>
    </section>
  );
...
function Footer() {{
  return (
    <section class="py-16 bg-neutral-900 border-t border-neutral-800 px-6">
      <div class="max-w-6xl mx-auto flex flex-col sm:flex-row justify-between items-center">
        <p class="text-neutral-400">{footer_tagline}</p>
        <div class="flex gap-6 mt-4 sm:mt-0">
          <a href="#" class="text-neutral-400 hover:text-{accent_light} transition-colors">Privacy</a>
          <a href="#" class="text-neutral-400 hover:text-{accent_light} transition-colors">Terms</a>
          <a href="#" class="text-neutral-400 hover:text-{accent_light} transition-colors">Contact</a>
        </div>
      </div>
    </section>
  );
